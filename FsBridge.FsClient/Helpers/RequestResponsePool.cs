...
namespace FsBridge.FsClient.Helpers
{
    internal class RequestResponsePool
    {
        ...
        internal void AppendRequest(Guid requestId, Type command, Guid? callId, Action<CommandReply> callBack = null)
        {
            if (!_queries.TryAdd(requestId, new _RequestResponseEntry() { CallId = callId, CallBack = callBack, RequestedOn = DateTime.Now })) throw new Exception("Cannot Add request to dictionary!");
        }
